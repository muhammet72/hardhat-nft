  ...
  : describe("Random NFT Unit Test", async () => {
    ...
    describe("constructor", async () => {
        it("sets starting values correctly", async () => {
            const dogUriZero = await randomIpfsNft.getDogTokenUris(0)
            const isInitialized = await randomIpfsNft.getInitialized()
            assert(dogUriZero.includes("ipfs://"))
    ...
    describe("Request Nft", async () => {
        it("fails if payment isn't sent with the request", async () => {
            await expect(randomIpfsNft.requestNft()).to.be.rejectedWith("RandomIpfsNft__NeedMoreETHSent")
        })
        ...
        it("reverts if payment amount is less than the mint fee", async () => {
            ...
            await expect(
                randomIpfsNft.requestNft({
                    value: fee.sub(await ethers.utils.parseEther("0.001")),
                })
            ).to.be.rejectedWith("RandomIpfsNft__NeedMoreETHSent")
        })

        it("emit event and kick off random words request", async () => {
            const fee = await randomIpfsNft.getMintFee()
